 ...
 export interface renderColInteface{
   ...
   */
   export interface renderCellInteface{
      ...
      * @param:table,当前表格
      */
    (this:SlColumn,rowData:any,index:number):TemplateResult<1>|{
 ...
 */
 export type ColumnHeaderData={
     ...
     * 自定渲染 td 
     */
    renderTd? :(this:SlColumn,rowData:any, index:number,tab:SlTable)=>TemplateResult<1>|{
 ...
 */
 export const convertHeaderDataToTableColumns=(columns:ColumnHeaderData[],table:SlTable)=>{
    ...
    const iteratorData=(_parent:ColumnHeaderData|null,childs:ColumnHeaderData[] ,parentDom:Element|DocumentFragment )=>{
        childs.forEach((item) =>{
