...
export const checkboxColChange = (table: SlTable, checkbox: SlCheckbox) => {
    ...
    if (checked) {
        const doTreeNode = (nodeData: any) => {
            ...
            const checkedValue = table.checkPropField ? nodeData[table.checkPropField] : nodeData;
...
export const checkboxTDChange = (checkbox: SlCheckbox, table: SlTable) => {
    ...
    const checkedValue = table.checkPropField ? rowData[table.checkPropField] : rowData;
...
registerCellTemplate('checkbox', (context, table) => {
    ...
    return html`<sl-checkbox class='tdCheckbox' @sl-before-change=${(event: Event) => {
    ...
    }} .checked=${live(ischecked)} .disabled=${isdisabled}   @sl-change=${(event: Event) => checkboxTDChange(event.target as SlCheckbox, table)}></sl-checkbox>`;
});
