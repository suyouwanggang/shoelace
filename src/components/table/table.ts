...
import caculateColumnData, { clearColumnCacheData, getColumnCacheData, getFieldValue, isNumberWidth, RowHeader } from './tableHelper';
/**
 * @since 2.0
 * @status experimental
 *
  ...
  */
  ...
  render() {
    ...
    return html`<div class='sl-table-base' part='base'  size=${this.size}>
        <div class='sl-table-base-scroll-div' style=${this.tableHeight?`height:calc( ${this.tableHeight} )`:''} @scroll=${this.handerScroll} @mousewheel=${this.handerScroll}  part='scroll-div'
         ?hover-able=${this.hoverAble} ?stripe=${this.stripe} ?border=${this.border}  >
          <!--渲染table 区 -->
            <table part='table' id='tableID'>
              <thead part='thead-hidden'>
                ${this.renderTheadRows(false)}
              </thead>
              <tbody>${this.renderDataSourceRows()}</tbody>
            </table>
          <!--渲染tableHeader 区 -->
          <div part='table-header-div'>
             <table part='fixed-thead-table'>
                <thead part='thead-fixed'>${this.renderTheadRows(true)}</thead>
             </table>
          </div>
        </div>
  ...
  private renderTheadRows(fixed:boolean){
    ...
    const renderTH=(colData:SlColumn)=>{
      ...
      return html`<th style=${styleMap(styleObject)} .column=${colData}  .cacheData=${cacheData} .rowSpan=${cacheData.rowspan as number} .colSpan=${cacheData.colspan as number}
  ...
  private renderDataSourceRows(){
    ...
    if(dataSource){
      for(let index=0,j=dataSource.length;index<j;index++){//行循环
        let rowData=dataSource[index];
        const rowHtml=[];
        for(let x=0,y=cellTdArray.length;x<y;x++){//TD循环
            ...
            if(tdResult==nothing||(tdResult.rowspan==0||tdResult.colspan==0)){
              ...
              >
                ${tdResult.template?tdResult.template:tdResult}</td>
              `);
            }
        }
        rowList.push(html`<tr .data=${rowData}>${rowHtml}</tr>`);
      }
    }
    return  rowList;
  }
