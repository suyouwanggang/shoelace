...
var componentID = 0;
var SlTable = class extends LitElement {
  constructor() {
    super(...arguments);
  ...
  _renderRowDataBetween(start, end) {
    ...
    for (let i = start, j = end; i < j; i++) {
      ...
      for (let x = 0, y = cellTdArray.length; x < y; x++) {
        ...
        if (tdResult != nothing && tdResult != null && tdResult != void 0) {
      ...
      const expandRowTemplate = this.expandRowRender != void 0 && this.expandRowData.includes(rowData) ? this.expandRowRender(rowContext, cellTdArray, this.cacheExpandLazyLoadDataMap.get(rowData)) : nothing;
      rowList.push(html`<tr
            ${ref((el) => {
...
__decorateClass([
  property({ type: String, reflect: true, attribute: "cache-key" })
