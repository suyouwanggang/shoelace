...
var renderSortHeaderTemplate = (table, column, hander) => {
  ...
  if (column.sortAble) {
    ...
    if (table.sortConfig.alwaysShowIcon) {
      result = $`<div class="sort-wrap" @click=${hander}>
        <div class="sort-ASC  ${sortResult && sortResult.orderType == "ASC" /* ASC */ ? "current" : ""}">${sortUp}</div>
        <div class="sort-DESC ${sortResult && sortResult.orderType == "DESC" /* DESC */ ? "current" : ""}">${sortDown}</div>
      </div>`;
    } else {
      result = $`<div class="sort-wrap" @click=${hander}>
        <div class="sort-${sortResult ? sortResult.orderType : "null"} current">
          ${sortResult && sortResult.orderType == "ASC" /* ASC */ ? sortUp : sortResult && sortResult.orderType == "DESC" /* DESC */ ? sortDown : ""}
        </div>
      </div>`;
    }
  }
