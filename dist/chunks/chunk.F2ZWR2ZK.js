...
var defaultColMap = /* @__PURE__ */ new Map();
...
registerColTemplate("checkbox", (_column, table) => {
  ...
  return $`<sl-checkbox class="table-check" .checked=${checkAll} .indeterminate=${indeterminate} @sl-change=${(event) => checkboxColChange(table, event.target)}></sl-checkbox>`;
});
var getColumnRenderResult = (context, table) => {
  const column = context.column;
  if (column.renderCol) {
    return $`${column.renderCol(context)}`;
  }
...
var defaultCellMap = /* @__PURE__ */ new Map();
