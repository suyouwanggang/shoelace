import {
  addResizeHander
} from "./chunk.2OUZ2E4D.js";
import {
  gallery_styles_default
} from "./chunk.GQ5ZVKXM.js";
import {
  watch
...
var SlGallery = class extends s {
  ...
  renderThumbimages() {
    ...
    const to = (_a = this.thumb_images) == null ? void 0 : _a.map((item, index) => {
      return $`<button part="thumb-button" @click=${() => this.goImageByChange(index - this.currentIndex)} class="thumb-button " ?current-image=${this.currentIndex == index}>
        <img part="thumb-image" class="thumb-image" .src=${item} />
      </button>`;
    });
    return $`<div class="thumb-image-conatainer" id="thumb-image-conatainer">${this._loadedOneImage ? to : w}</div>`;
  }
  renderImages() {
    var _a;
    const itemRender = (item, index) => {
      var _a2;
      return $`<div>
        ${!this.layImage || Math.abs(this.currentIndex - index) <= 1 ? $`<img
              @load=${(event) => {
        ...
        emit(this, "sl-gallery-image-click", {
          detail: { image: event.target }
              ...
              title=""
            />` : w}
        ${index == this.currentIndex && this.imageRender && this.image_datas ? $`<div part="image-exta">${(_a2 = this.imageRender) == null ? void 0 : _a2.call(this, this.image_datas[index], index)}</div>` : w}
      </div>`;
    };
    ...
    return $`<div class="image-sliders">${to}</div>`;
  }
  renderImgeNavigations() {
    ...
    return $`<div class="imgage-navigation" part="image-naviagation">
      <div class="imgage-navigation-wrap" part="image-naviagation-wrap">
        ${(_a = this.images) == null ? void 0 : _a.map((_item, index) => {
      return $`<button ?current-image=${index == this.currentIndex} part="nav-button" @click=${() => this.goImageByChange(index - this.currentIndex)}></button>`;
    })}
      </div>
    </div>`;
  }
  ...
  renderFullScreenButton() {
    ...
    return this.show_fullscreen ? $`<button class="nav-button button-fullscreen" @click=${this.changeFullScreenState} part="full-screen">${this.isFullScreened ? svgFullscreened : svgFullscreen}</button>` : "";
  }
