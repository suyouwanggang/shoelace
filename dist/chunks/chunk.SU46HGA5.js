...
var SlGallery = class extends LitElement {
  ...
  renderNavLefAndRight() {
    return this._loadedOneImage && this.showNavButtons ? html`
  ...
  renderImages() {
    ...
    const itemRender = (item, index) => {
      var _a2;
      return html`<div>
        ${!this.layImage || Math.abs(this.currentIndex - index) <= 1 ? html`<img
              @load=${(event) => {
        ...
        emit(this, "sl-gallery-image-click", {
          detail: { image: event.target }
              ...
              title=""
            />` : nothing}
        ${index == this.currentIndex && this.imageRender && this.image_datas ? html`<div part="image-exta">${(_a2 = this.imageRender) == null ? void 0 : _a2.call(this, this.image_datas[index], index)}</div>` : nothing}
      </div>`;
    };
  ...
  renderPauseButton() {
    ...
    return this.show_pause ? html`<button
...
__decorateClass([
  property({ type: String, attribute: "thumb-position" })
