...
var SlDatePanel = class extends n {
  constructor() {
    super(...arguments);
    this.mode = "date";
    this.innerMode = "date";
  ...
  renderHeader() {
    return y`<button class="button" part="prevButton" id="prevButton">
  ...
  renderDatePanel() {
    ...
    const weeks = y`${weekArray.map((item) => y`<span class="date-week-item" part="week-head">${item}</span>`)} `;
    ...
    for (let i = 0, j = dateArray.length; i < j; i++) {
      ...
      dateResult.push(y`<button part="item-date" class="date-button date-day-item ${other ? "other" : ""} ${isCurent ? "current" : ""} " .date=${tempDate} .disabled=${disabled}>${tempDate.getDate()}</button>`);
    }
    return y`<div class="date-body">${weeks}${dateResult}</div>`;
  }
  renderMonthBody() {
    ...
    if (current) {
      ...
      return month.map((value, index) => {
        ...
        return y`<button part="item-month" .date=${date} class="date-button date-day-item ${isEqualsDate(date, this.valueDate, "month") ? "current" : ""}" ?disabled=${disabled}>${value}</button>`;
      });
    }
    return x;
  }
  ...
  renderYearBody() {
    ...
    if (current) {
      ...
      const n3 = parseInt(String(nv / 20));
      const year = n3 * 20;
      const minDate = this.minDate;
      const maxDate = this.maxDate;
      for (let i = year, j = year + 20; i < j; i++) {
        ...
        result.push(y`<button part="item-year" .date=${date} class="date-button date-day-item ${isEqualsDate(date, this.valueDate, "year") ? "current" : ""}" ?disabled=${disabled}>${i}</button>`);
      }
    }
    return result;
  }
  ...
  render() {
    return y`<div class="date-base" part="base">
      <div class="date-head">${this.renderHeader()}</div>
      <div part="panel-base">
        ${this.innerMode == "date" ? y`<div part="date-date" class="date-panel date-date ">${this.renderDatePanel()}</div>` : x}
        ${this.innerMode == "month" ? y`<div part="date-month" class="date-panel date-month">${this.renderMonthBody()}</div>` : x}
        ${this.innerMode == "year" ? y`<div part="date-year" class="date-panel date-year ">${this.renderYearBody()}</div>` : x}
      </div>
    </div>`;
  }
};
