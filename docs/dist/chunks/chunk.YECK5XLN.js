...
import {
  editNone,
  n as n2
} from "./chunk.K3WIG5HP.js";
import {
  table_styles_default
} from "./chunk.PY2KS52Y.js";
import {
  removeTableCacheByKey,
  restoreFromLocalCache,
  saveAsDefaultTableCache,
  updateTableCache
} from "./chunk.QMLDBTX4.js";
import {
  defaultSortConfig,
  defaultTreeConfig
} from "./chunk.PWW43EL5.js";
import {
  debounce,
  throttleTimeout
...
var rowContextMap = /* @__PURE__ */ new WeakMap();
...
var SlTable = class extends s {
  constructor() {
    ...
    this.cacheExpandLazyLoadDataMap = /* @__PURE__ */ new Map();
  ...
  _renderNoDataTemplate() {
    if (this.innerDataSource && this.innerDataSource.length == 0) {
      return $`<slot @slotchange=${this.columnChangeHanlder} name="no-data">${getResouceValue("noData")}</slot>`;
    }
    return ``;
  }
  ...
  watchDataSourceChange() {
    if (this.treeConfig && this.dataSource) {
      ...
      this.cacheTreeNodeMap = /* @__PURE__ */ new Map();
      let allTreeNode = [];
      let result = [];
      let seqNo = 0;
  ...
  _renderRowDataBetween(start, end) {
    ...
    for (let i3 = start, j = end; i3 < j; i3++) {
      ...
      for (let x = 0, y = cellTdArray.length; x < y; x++) {
        ...
        if (tdResult != w && tdResult != null && tdResult != void 0) {
      ...
      const expandRowTemplate = this.expandRowRender != void 0 && this.expandRowData.includes(rowData) ? this.expandRowRender(rowContext, cellTdArray, this.cacheExpandLazyLoadDataMap.get(rowData)) : w;
      rowList.push($`<tr
            ${n2((el) => {
