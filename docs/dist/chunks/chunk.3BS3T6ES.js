...
var SlTree = class extends n {
  ...
  renderAllTreeNode() {
    if (!this.renderRootNodeData) {
      return y`<slot name="no-data"></slot>`;
    } else {
      if (this.includeRoot) {
        return this.renderNodeDataTemplate(this.renderRootNodeData, 0);
      } else {
        const children = this.renderRootNodeData.children;
        return children ? children.map((item, index) => this.renderNodeDataTemplate(item, index, this.renderRootNodeData)) : x;
      }
    }
  }
  renderNodeDataTemplate(data, index, parentData) {
    const tree = this;
    return y`<sl-tree-node .customStyle=${this.customStyle} .tree=${tree} .nodeData=${data} index=${index} .parentNodeData=${parentData} .nodeRender=${this.real_treeNodeRender}></sl-tree-node>`;
  }
  ...
  render() {
    ...
    return y`<div part="base" class=${o(baseClass)}>
      ${this.enableFilter ? y`<div part="filter">
      ...
      ${this.loading ? y`<div class="modal" part="modal">
  ...
  watchNodeRenderChange() {
    ...
    this.real_treeNodeRender = (node, index, parentNodeData) => {
      ...
      return y`<div part="select-part" @click=${handerNodeSelect}>${array}</div>`;
    };
  }
