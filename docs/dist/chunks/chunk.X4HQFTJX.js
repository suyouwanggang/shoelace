...
registerColTemplate("checkbox", (_column, table) => {
  ...
  return y`<sl-checkbox class="table-check" .checked=${checkAll} .indeterminate=${indeterminate} @sl-change=${(event) => checkboxColChange(table, event.target)}></sl-checkbox>`;
});
var getColumnRenderResult = (context, table) => {
  const column = context.column;
  if (column.renderCol) {
    return y`${column.renderCol(context)}`;
  }
  let templateFun = getReisterColTemplate(column.type);
  if (templateFun) {
    return templateFun(column, table);
  } else {
    return y`${column.label}`;
  }
};
