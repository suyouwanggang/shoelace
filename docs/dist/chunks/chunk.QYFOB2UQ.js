...
(function(w) {
  ...
  function y(t, c) {
    ...
    for (r2 = 0; r2 < e2; r2 += 1)
      for (b = 0; b < e2; b += 1) {
        var g = a, x = c.left + b * h, u = c.top + r2 * h, p2 = r2, q = b, f = d.a, k = x + h, m = u + h, D = p2 - 1, E = p2 + 1, n2 = q - 1, l = q + 1, y2 = Math.floor(Math.min(0.5, Math.max(0, c.R)) * h), v2 = f(p2, q), I = f(D, n2), w2 = f(D, q);
        D = f(D, l);
        var F = f(p2, l);
        l = f(E, l);
        q = f(E, q);
  ...
  function C(c, a) {
    ...
    function b(b2, a2) {
        ...
        for (m = 0; m < f.length; m += 1) {
          ...
          if (r2[p2][q] == null)
            for (var n2 = -2; 2 >= n2; n2 += 1)
              for (var l = -2; 2 >= l; l += 1)
                r2[p2 + n2][q + l] = n2 == -2 || n2 == 2 || l == -2 || l == 2 || n2 == 0 && l == 0;
        }
      for (f = 8; f < d - 8; f += 1)
      ...
      for (q = d - 1; 0 < q; q -= 2)
        for (q == 6 && --q; ; ) {
          for (n2 = 0; 2 > n2; n2 += 1)
            r2[k][q - n2] == null && (l = false, p2 < b2.length && (l = (b2[p2] >>> m & 1) == 1), a2(k, q - n2) && (l = !l), r2[k][q - n2] = l, --m, m == -1 && (p2 += 1, m = 7));
          k += f;
          if (0 > k || d <= k) {
            k -= f;
            f = -f;
