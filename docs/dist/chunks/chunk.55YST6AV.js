...
registerColTemplate("checkbox", (_column, table) => {
  ...
  return p`<sl-checkbox class="table-check" .checked=${checkAll} .indeterminate=${indeterminate} @sl-change=${(event) => checkboxColChange(table, event.target)}></sl-checkbox>`;
});
var getColumnRenderResult = (context, table) => {
  const column = context.column;
  if (column.renderCol) {
    return p`${column.renderCol(context)}`;
  }
