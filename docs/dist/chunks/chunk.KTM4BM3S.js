    ...
    return y`
      <div class="org-tree-node-label " part="org-tree-node-label">
        <div class="org-tree-node-label-inner ${this.styleClass ? this.styleClass : ""}" @click=${this.onNodeClick}>
          ${this.nodeRender(this.nodeData)} ${!isLeaf && this.collapsable ? y`<span class="org-tree-node-btn ${this.expanded ? "expanded" : ""}" @click=${this.onToogleNode}></span>` : ""}
        </div>
      </div>
      ${!isLeaf && this.expanded ? y`<div class="org-tree-node-children">${this._renderChildNode()}</div>` : ""}
    `;
  }
  onNodeClick() {
  ...
  _renderChildNode() {
    ...
    if (this.nodeData.children) {
      ...
      for (let i = 0, j = child.length; i < j; i++) {
        ...
        result.push(y`<sl-org-node class=${o(classObj)} .nodeRender=${this.nodeRender} .nodeData=${subNode} .expanded=${expanded} .collapsible=${collapsible}></sl-org-node>`);
      }
    }
    return result;
  }
