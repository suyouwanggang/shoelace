import {
  ...
  isCellEditor,
  n
} from "./chunk.3FXL7HWB.js";
import {
  renderSortHeaderTemplate,
  sortRenderHanlder
} from "./chunk.B6FHON36.js";
import {
  SortTrigger
...
var renderThColTemplate = (context, table) => {
  ...
  >
    <div class="thWrap">
      ...
      ${renderSortHeaderTemplate(table, column, handerSort)} ${column.resizeAble ? p`<div part="resize-hanler" @click=${stopHanderClick} class="th-resize-helper"></div>` : ""}
    </div>
  </th>`;
};
...
var renderTdCellTemplate = (context, table) => {
  ...
  if (tdResult == void 0 || tdResult == T || tdResult.rowspan == 0 || tdResult.colspan == 0) {
    return T;
  } else {
    ...
    const tdHtml = p`<div class="tdWrap ${!isEditFlag && table.enableCellBox && column.type != "checkbox" ? "cellBox" : ""}">${tdResult.template ? p`${tdResult.template}` : p`${tdResult}`}</div>`;
...
var wrapColumnFieldTemplate = (context, wrap, table) => {
  ...
  if (column.field && table.treeConfig && column.field == table.treeConfig.treeNodeColumn) {
    ...
    let span = p`<span class="tree-node-icon" componentID=${table.componentID} part="tree-node-toogle"
      >${table.treeLoadingNode.includes(rowData) ? p`<sl-spinner part="tree-node-loading"></sl-spinner>` : closed ? TABLESVG.closeNodeSvg : TABLESVG.openNodeSvg}
    </span>`;
    return p`<div part="tree-node" class="tree-node ${table.treeNodeNoWrap ? "nowrap-tree-node" : ""} ${closed ? "closed" : ""}">
      <div class="tree-node-inner" style="padding-left:${level * table.treeConfig.indent}px;">
        ${table.treeNodeHasChildren(rowData) ? span : p`<span class="tree-node-empty-node"></span>`} ${rowData.icon ? p`<sl-icon class="tree-node-icon" name=${rowData.icon}></sl-icon>` : ""} ${wrap}
      </div>
    </div>`;
  } else if (column.field && table.expandColumn && column.field == table.expandColumn) {
    const opened = table.expandRowData.includes(rowData);
    return p`<div class="expand-toogle" part="expand-wrap">
      <span class="expand-toogle-icon" componentID=${column.table.componentID} part="expand-toogle-icon">
        ${table.expandingRowData.includes(rowData) ? p`<sl-spinner part="expand-loading"></sl-spinner>` : opened ? TABLESVG.expendOpendSvg : TABLESVG.expendCloseSvg}
      </span>
      ${wrap}
    </div>`;
  }
