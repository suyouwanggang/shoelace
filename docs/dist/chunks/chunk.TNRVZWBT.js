...
var n2 = e(class extends d {
  ...
  update(i2, [s]) {
    var e4;
    const o3 = s !== this.U;
    return o3 && this.U !== void 0 && this.nt(void 0), (o3 || this.rt !== this.lt) && (this.U = s, this.ht = (e4 = i2.options) === null || e4 === void 0 ? void 0 : e4.host, this.nt(this.lt = i2.element)), x;
  }
  nt(t2) {
...
registDefaultEditor(EDIT_TYPE.SELECT, (context) => {
  ...
  return y`<sl-select
    ...
    .value=${l(value)}
  >
    ${column.items ? column.items.map((item) => y`<sl-menu-item .value=${item.id}>${getSelectLable(item)}</sl-menu-item>`) : ""}
  </sl-select>`;
});
registDefaultEditor(EDIT_TYPE.MULIT_SELECT, (context) => {
  ...
  >
    ${column.items ? column.items.map((item) => y`<sl-menu-item .value=${l(item.id)}>${getSelectLable(item)}</sl-menu-item>`) : ""}
  </sl-select>`;
});
registDefaultEditor(EDIT_TYPE.MULIT_CHECKBOX, (context) => {
  ...
  >
    ${column.items ? column.items.map((item) => y`<sl-checkbox .checked=${l(array.includes(item.id))} .value=${item.id}>${getSelectLable(item)}</sl-checkbox>`) : ""}
  </div>`;
});
