...
var SlTransfer = class extends n {
  constructor() {
    super(...arguments);
  ...
  renderFilter(_direction) {
    ...
    return y`<sl-input direction=${_direction} size="small" @sl-input=${(event) => this.doFilterInputHandler(event, _direction)} placeholder=${placeholder} exportparts="base:slInput,input:input" clearable
  ...
  tableTemplateChange() {
    ...
    A(this.tableTemplate("source"), frag);
    ...
    A(this.tableTemplate("target"), targetfrag);
  ...
  renderContent(direction) {
    ...
    if (this.tableTemplate != void 0) {
      ...
      return y`${direction == "source" ? this.sourceTable : this.targetTable}`;
    }
    const result = items.map((item) => {
      return y`<li class="render-item" part="render-item" @click=${(event) => this.processSelectItem(event, item, direction)}>
        <sl-checkbox
    ...
    if (!items || items.length == 0) {
      return y`<div slot=${direction + "-empty"} class="emptyData">${getResouceValue("noData")}</div>`;
    } else {
      return y`<sl-scroll ${n3(direction == "source" ? this.sourceScrollRef : this.targetScrollRef)}>${result}</sl-scroll>`;
    }
  }
  render() {
    return y`<div part='base'>
      <div part='container' class='source' >
        <div part='header'></div>
        <div part='body'>
          ${!this.disableFilter ? y`<div part="search">${this.renderFilter("source")}</div>` : x}
      ...
      <div part='container' class='target'>
        <div part='header'></div>
        <div part='body'>
            ${!this.disableFilter ? y`<div part="search">${this.renderFilter("target")}</div>` : x}
